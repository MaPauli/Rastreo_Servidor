jQuery(document).ready(function($) {
    ...
    function getAllRoutesForMap() {
        ...
        showPermanentMessage('Seleccione una Ruta ');
    ...
    function loadRoutes(json) {        
        if (json.length == 0) {
            showPermanentMessage('No hay rutas disponibles para ver');
        }
        else {
            // create the first option of the dropdown box
            var option = document.createElement('option');
            option.setAttribute('value', '0');
            option.innerHTML = 'Seleccione una ruta...';
    ...
    function loadGPSLocations(json) {
        ...
        if (json.length == 0) {
            showPermanentMessage('No hay rutas disponibles');
            map.innerHTML = '';
        }
        else {
    ...
    function createMarker(latitude, longitude, speed, direction, distance, locationMethod, gpsTime,
        ...
        if (finalLocation) {
            lastMarker = "</td></tr><tr><td align=left>&nbsp;</td><td><b>Localización Final</b></td></tr>";
        }
        ...
        var popupWindowText = "<table border=0 style=\"font-size:95%;font-family:arial,helvetica,sans-serif;color:#000;\">" +
            ...
            "<tr><td align=right>Velocidad:&nbsp;</td><td>" + speed +  " mph</td></tr>" +
            "<tr><td align=right>Distancia:&nbsp;</td><td>" + distance +  " mi</td><td>&nbsp;</td></tr>" +
            "<tr><td align=right>Hora:&nbsp;</td><td colspan=2>" + gpsTime +  "</td></tr>" +
            "<tr><td align=right>Nombre:&nbsp;</td><td>" + userName + "</td><td>&nbsp;</td></tr>" +
            "<tr><td align=right>Exactitud:&nbsp;</td><td>" + accuracy + " ft</td><td>&nbsp;</td></tr></table>";
    ...
    function turnOffAutoRefresh() {
        showMessage('Actualizacion Desactivada');
        $('#autorefresh').val('Actualización desactivada');
    ...
    function turnOnAutoRefresh() {
        showMessage('Actualizacion activada (1 min)'); 
        $('#autorefresh').val('Actualización activada');
        autoRefresh = true;
